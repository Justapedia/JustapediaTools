...
import {
  fetchRandomInfoboxArticle,
  processInfoboxSync,
  saveEdit,
} from "@/utils/updateBotLogic";
...
export default function UpdateBotPage() {
  ...
  useEffect(() => {
    if (visitedArticles.current.size > 5000) {
      ...
      setTimeout(() => {
        addLog(
          "Cleared visited articles cache for memory optimization",
          "System",
          "info",
        );
      }, 0);
    ...
    try {
      ...
      if (botUser) {
        setStats({
          ...
          registration: botUser.registration
            ? new Date(botUser.registration).toLocaleDateString()
            : "N/A",
  ...
  const runBotCycle = async () => {
    ...
    try {
      ...
      const article = await fetchRandomInfoboxArticle(
        categories,
        Array.from(visitedArticles.current),
      );

      if (!article) {
        addLog(
          "Failed to fetch article or no candidates found",
          "Bot",
          "warning",
        );
        ...
        <div>
          ...
          <div className="text-zinc-400 max-w-2xl text-sm space-y-4">
            <p className="leading-relaxed">
              Syncs infobox data from English Wikipedia into Justapedia
              articles. Configure categories, review proposed changes, and run
              in auto-save or manual review mode.
            </p>
          </div>
        </div>
          ...
          <div className="flex items-center gap-3 bg-zinc-900 p-2 rounded-xl border border-zinc-800 h-fit justify-end">
            {!isRunning ? (
              ...
              >
                ...
                {processAll ? "Start Continuous Mode" : "Start Bot"}
              </button>
            ) : (
              <button
                onClick={handleStop}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="flex items-center justify-between p-3 bg-zinc-950 rounded-lg border border-zinc-800">
                <div>
                  ...
                  <div className="text-xs text-zinc-500">
                    Automatically save changes (Required for high-speed
                    operation)
                  </div>
                </div>
                <button
                ...
                >
                  <div
                    className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-transform ${autoSave ? "left-7" : "left-1"}`}
                  />
                </button>
              </div>
            </div>
        ...
        <div className="lg:col-span-2 bg-zinc-900/50 border border-zinc-800 rounded-xl p-6">
          ...
          <div className="text-sm text-zinc-400">
            <div className="space-y-2">
              <h4 className="font-bold text-zinc-300 uppercase text-xs tracking-wider">
                Goal
              </h4>
              <p>
                The goal is Infobox consistency only, not full article
                synchronization.
              </p>
            </div>
          </div>
        ...
        <div className="lg:col-span-3 bg-zinc-900 border border-zinc-800 rounded-xl p-6 h-[400px] flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto space-y-2 font-mono text-xs pr-2">
            {logs.map((log) => (
              <div
                key={log.id}
                className="flex gap-4 p-2 rounded bg-zinc-950/50 border border-zinc-900/50"
              >
