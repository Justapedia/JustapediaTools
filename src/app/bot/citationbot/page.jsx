...
export default function CitationBotPage() {
  ...
  const runBotCycle = async () => {
    ...
    if (scanModeRef.current === "priority") statusMsg = "Scanning for priority issues...";
    if (scanModeRef.current === "category") statusMsg = `Scanning category: ${targetCategoryRef.current}...`;
    if (scanModeRef.current === "page") statusMsg = `Fetching page: ${targetPageRef.current}...`;
    ...
    try {
        ...
        if (scanModeRef.current === "priority") {
            ...
            if (!article) {
                addLog("No priority issues found, checking random...", "Sourav bot", "info");
            ...
            if (!article) {
                 addLog(`No articles found in category: ${targetCategoryRef.current}`, "Sourav bot", "warning");
                 setIsRunning(false);
                 return;
            }
            ...
            if (!article) {
                 addLog(`Page not found: ${targetPageRef.current}`, "Sourav bot", "error");
                 setIsRunning(false);
                 return;
            }
        } else {
        ...
        if (result.unfixableIssues && result.unfixableIssues.length > 0) {
             addLog(`Found ${result.unfixableIssues.length} unfixable issues`, "Sourav bot", "warning");
             if (autoSaveRef.current) {
                 await postTalkPageReport(article.title, result.unfixableIssues);
                 addLog(`Posted report to Talk page`, "Sourav bot", "success");
             } else {
                 addLog(`Manual intervention required: ${result.unfixableIssues.join(", ")}`, "Sourav bot", "warning");
             }
        }

        if (!result.hasChanges) {
            // Check for unfixable issues in Manual Mode
            if (result.unfixableIssues && result.unfixableIssues.length > 0 && !autoSaveRef.current) {
                 setPendingEdit({
                    title: article.title,
            ...
            if (performNullEditsRef.current) {
                 addLog(`Performing Null Edit for maintenance on ${article.title}...`, "Sourav bot", "warning");
                 const saveRes = await saveEdit(article.title, article.content, "Bot: Null edit for maintenance category cleanup", true);
                 
                 if (saveRes.success) {
                    addLog(`Saved (Null Edit): ${article.title}`, "Sourav bot", "success");
                 } else {
        ...
        if (autoSaveRef.current) {
            // Auto-save mode
            addLog(`Applying fixes to ${article.title}...`, "Sourav bot", "warning");
        ...
        <div className="flex flex-col gap-4">
            <div className="flex flex-col gap-3 bg-zinc-900 p-4 rounded-xl border border-zinc-800">
                ...
                <div className="flex flex-wrap items-center gap-2 bg-zinc-950/50 p-2 rounded-lg border border-zinc-800/50">
                    <span className="text-xs font-bold text-zinc-500 uppercase px-2">Scan Mode:</span>
            ...
            <div className="flex justify-end">
              {!isRunning ? (
                  <button
                    onClick={handleStart}
                    disabled={(scanMode === "category" && !targetCategory.trim()) || (scanMode === "page" && !targetPage.trim())}
                    className={`flex items-center gap-2 px-6 py-2 rounded-lg font-bold transition-all shadow-lg ${
                        (scanMode === "category" && !targetCategory.trim()) || (scanMode === "page" && !targetPage.trim())
      ...
      {pendingEdit && (
          <div className="bg-amber-500/10 border border-amber-500/30 rounded-xl p-6 animate-in fade-in slide-in-from-top-4">
              ...
              <div className="space-y-4">
                  ...
                  {pendingEdit.unfixableIssues && pendingEdit.unfixableIssues.length > 0 && (
                      <div className="bg-orange-500/10 p-4 rounded-lg border border-orange-500/20">
                          <div className="flex items-center justify-between mb-2">
                              <h4 className="text-xs font-bold text-orange-400 uppercase flex items-center gap-2">
                                  <AlertTriangle className="w-3 h-3" /> Requires Manual Intervention
                              </h4>
                              <button
                  ...
                  <div className="bg-zinc-900 p-4 rounded-lg border border-zinc-800">
                      <h4 className="text-xs font-bold text-zinc-500 uppercase mb-2">Proposed Changes</h4>
                  ...
                  <div className="grid grid-cols-2 gap-4 h-[300px]">
                      <div className="bg-zinc-950 p-3 rounded-lg border border-zinc-800 overflow-auto">
                          <div className="text-xs font-bold text-red-400 mb-2 sticky top-0 bg-zinc-950 pb-2 border-b border-zinc-900">Original</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-3 bg-zinc-900/50 border border-zinc-800 rounded-xl p-6">
            ...
            <p className="text-zinc-400 mb-6 leading-relaxed">
                Please search the entire site and identify all pages where citation-related problems exist and cannot be efficiently fixed manually.
            </p>
            ...
            <div className="grid md:grid-cols-2 gap-8 text-sm">
                ...
                <div className="space-y-4">
                    ...
                    <p className="text-zinc-500 italic mb-2">These are the types of issues that specifically require the use of the Citation Bot:</p>
            ...
            <div className="mt-8 pt-6 border-t border-zinc-800">
                ...
                <p className="text-zinc-400 mb-3 text-sm">By identifying and addressing these pages:</p>
                <div className="grid md:grid-cols-3 gap-4">
                    {[
                        ...
                        <div key={i} className="bg-emerald-500/5 border border-emerald-500/10 p-3 rounded-lg text-emerald-300 text-sm font-medium flex items-center gap-2">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden">
            ...
            <div className="max-h-[300px] overflow-y-auto p-2 space-y-1">
              {logs.map((log) => (
                <div key={log.id} className="flex flex-col p-2 rounded hover:bg-zinc-800/50 text-sm font-mono transition-colors border-b border-zinc-800/50 last:border-0">
                  <div className="flex items-center gap-3">
                    <span className="text-zinc-500 text-xs min-w-[60px]">{log.time}</span>
                    ...
                    <span className="text-zinc-300 flex-1 truncate">
                        <span className="font-bold text-white mr-2">[{log.user}]</span>
                        {log.action}
                    </span>
                  </div>
                  {log.details && (
